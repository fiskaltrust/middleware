...
namespace fiskaltrust.service.launcher.Helpers.Grpc.Interceptors
{
    public class ServerLoggingInterceptor : Interceptor
    {
        ...
        {
            string message = "grpc Request | Method type: {0} | Method name: {1} | Request type: {2} | Response type: {3}";
            if (request != null)
            {
                message += " | Response:" + System.Environment.NewLine + "{4}";
            }
            _logger.LogDebug(message, methodType, context.Method, typeof(TRequest), typeof(TResponse), JsonConvert.SerializeObject(request, Newtonsoft.Json.Formatting.Indented));
        }
        ...
        {
            string message = "grpc Response | Method type: {0} | Method name: {1} | Request type: {2} | Response type: {3} | Status: {4} ({5})";
            if (response != null)
            {
                message += " | Response:" + System.Environment.NewLine + "{6}";
            }
            _logger.LogDebug(message, methodType, context.Method, typeof(TRequest), typeof(TResponse), context.Status.StatusCode, context.Status.Detail, JsonConvert.SerializeObject(response, Newtonsoft.Json.Formatting.Indented));
        }
    }
}