...
namespace fiskaltrust.Middleware.SCU.DE.Epson.Helpers
{
    public static class LockingHelper
    {
        ...
        public static async Task PerformWithLock(SemaphoreSlim semaphore, Func<Task> method, int timeout = TIMEOUT_60_SECONDS)
        {
            var isLocked = false;
            try
            {
        ...
        public static async Task PerformWithLock(SemaphoreSlim semaphore, Action method, int timeout = TIMEOUT_60_SECONDS)
        {
            var isLocked = false;
            try
            {
