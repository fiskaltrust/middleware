...
namespace fiskaltrust.Middleware.SCU.DE.Swissbit.Interop
{
    public class SwissbitProxy : ISwissbitProxy, IDisposable
    {
        ...
        public async Task<FlashHealth> GetFlashHealthSummaryAsync()
        {
            return await _lockingHelper.PerformWithLock(_hwSemaphore, () =>
            {
                var uncorrectableEccErrorPtr = Marshal.AllocHGlobal(sizeof(UInt32));
                var percentageRemainingSpareBlocksPtr = Marshal.AllocHGlobal(sizeof(byte));
                var percentageRemainingEraseCountsPtr = Marshal.AllocHGlobal(sizeof(byte));
                var percentageRemainingTenYearsDataRetentionPtr = Marshal.AllocHGlobal(sizeof(byte));

                try
                {
                    _nativeFunctionPointer.func_worm_flash_health_summary(context, uncorrectableEccErrorPtr, percentageRemainingSpareBlocksPtr, percentageRemainingEraseCountsPtr, percentageRemainingTenYearsDataRetentionPtr).ThrowIfError();

                    var flashHealth = new FlashHealth
                    {
                        UncorrectableEccErrors = (UInt32) Marshal.ReadInt32(uncorrectableEccErrorPtr),
                        PercentageRemainingSpareBlocks = Marshal.ReadByte(percentageRemainingSpareBlocksPtr),
                        PercentageRemainingEraseCounts = Marshal.ReadByte(percentageRemainingEraseCountsPtr),
                        PercentageRemainingTenYearDataRetention = Marshal.ReadByte(percentageRemainingTenYearsDataRetentionPtr)
                    };

                    flashHealth.NeedsReplacement = _nativeFunctionPointer.func_worm_flash_health_needs_replacement(flashHealth.UncorrectableEccErrors, flashHealth.PercentageRemainingSpareBlocks, flashHealth.PercentageRemainingEraseCounts) != 0;

                    return flashHealth;
                }
                finally
                {
                    Marshal.FreeHGlobal(uncorrectableEccErrorPtr);
                    Marshal.FreeHGlobal(percentageRemainingSpareBlocksPtr);
                    Marshal.FreeHGlobal(percentageRemainingEraseCountsPtr);
                    Marshal.FreeHGlobal(percentageRemainingTenYearsDataRetentionPtr);
                }
            });
        }
