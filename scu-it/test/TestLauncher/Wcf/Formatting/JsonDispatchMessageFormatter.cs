#if NET461
using fiskaltrust.Middleware.SCU.IT.Test.Launcher.Wcf.Enums;
...
namespace fiskaltrust.Middleware.SCU.IT.Test.Launcher.Wcf.Formatting
{
    public class JsonDispatchMessageFormatter : IDispatchMessageFormatter
    {
        ...
        {
            if (message.Properties.TryGetValue("httpRequest", out var header) && header is HttpRequestMessageProperty property)
            {
                var contentTypeVals = property.Headers.GetValues("Content-Type");
