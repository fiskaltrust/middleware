...
namespace fiskaltrust.Middleware.SCU.IT.Test.Launcher.Wcf
{
    public class WCFService : IDisposable
    {
        ...
        public void ConfigureService(PackageConfiguration config, Type type, object instance, string uri)
        {
            ...
            try
            {
                ...
                _host.Open();
                _logger.LogInformation("{0}: {1} ({2}) - Endpoint: {3}", "WCF Service", config.Package, config.Version, uri);
            }
            catch (Exception x)
            {
                _logger.LogError(x, "Failed to start {0} {1} ({2}) - Endpoint: {3}", "WCF Service", config.Package, config.Version, uri);
            }
        }
            ...
            else
            {
                ...
                switch (baseAddress.Scheme)
                {
                    ...
                    case "net.pipe":
                        ...
                        break;
                    default:
                        _logger.LogInformation("No Endpoint could be provided with this Uri: {0}", uri);
                        break;
                }
            }
        }
