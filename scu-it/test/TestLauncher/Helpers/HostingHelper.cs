...
using System.Linq;
using fiskaltrust.Middleware.SCU.IT.Test.Launcher.Grpc;
using fiskaltrust.Middleware.SCU.IT.Test.Launcher.Wcf;
using fiskaltrust.storage.serialization.V0;
using Microsoft.Extensions.Logging;

namespace fiskaltrust.Middleware.SCU.IT.Test.Launcher.Helpers
{
    public static class HostingHelper
    {
        ...
        private static Type GetMiddlewareComponentTypeForPackage(PackageConfiguration config, object serviceInstance)
        {
            ...
            else if (config.Package.Contains("SCU.IT"))
            {
                return serviceInstance.GetType().GetInterfaces().First(x => x.FullName == typeof(ifPOS.v1.it.IITSSCD).FullName);
            }
