...
public sealed class BizkaiaTicketBaiSCU //: IESSSCD 
{
#pragma warning disable IDE0052
    ...
    private const string BIZKAIA_SUBMIT_ENDPOINT_DEV = "https://pruesarrerak.bizkaia.eus";
    private const string BIZKAIA_SUBMIT_ENDPOINT = "https://sarrerak.bizkaia.eus";

    public BizkaiaTicketBaiSCU(ILogger<TicketBaiSCU> logger, TicketBaiSCUConfiguration configuration)
    {
        _logger = logger;
        ...
        {
            BaseAddress = new System.Uri(BIZKAIA_SUBMIT_ENDPOINT_DEV)
        };
        _httpClient.DefaultRequestHeaders.Add("eus-bizkaia-n3-version", "1.0");
        _httpClient.DefaultRequestHeaders.Add("eus-bizkaia-n3-content-type", "application/xml");
    }

    public async Task SubmitInvoiceAsync()
    {
        var submitEndpoint = "/N3B4000M/aurkezpena";
        await _httpClient.PostAsync(submitEndpoint, new StringContent(""));
    }

    public async Task CancelInvoiceAsync()
    {
        var endpoint = "/N3B4000M/aurkezpena";
        await _httpClient.PostAsync(endpoint, new StringContent(""));
    }

    public async Task GetInvoicesAsync()
    {
        var endpoint = "/N3B4001M/kontsulta";
        await _httpClient.PostAsync(endpoint, new StringContent(""));
    }
