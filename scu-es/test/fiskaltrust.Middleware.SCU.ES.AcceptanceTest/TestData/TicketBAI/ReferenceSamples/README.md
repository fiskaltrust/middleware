...
NN [60] (`IE` foreign tax) maps from `ChargeItemCaseNatureOfVatES.ForeignTaxApplies` to `DesgloseFactura/NoSujeta/Causa=IE` with `ClaveRegimen=08` (per sample 021; the #95 table lists `01` but notes the regime is context-dependent). `NN60_ForeignTax_MatchesReference_IE_Clave08` asserts against sample 021.
