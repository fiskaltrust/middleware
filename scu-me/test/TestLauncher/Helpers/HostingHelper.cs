...
using System.Linq;
using fiskaltrust.Middleware.SCU.ME.Test.Launcher.Grpc;
using fiskaltrust.Middleware.SCU.ME.Test.Launcher.Wcf;
...
namespace fiskaltrust.Middleware.SCU.ME.Test.Launcher.Helpers
{
    public static class HostingHelper
    {
        ...
        private static Type GetMiddlewareComponentTypeForPackage(PackageConfiguration config, object serviceInstance)
        {
            ...
            else if (config.Package.Contains("SCU.ME"))
            {
                return serviceInstance.GetType().GetInterfaces().First(x => x.FullName == typeof(ifPOS.v1.me.IMESSCD).FullName);
            }
