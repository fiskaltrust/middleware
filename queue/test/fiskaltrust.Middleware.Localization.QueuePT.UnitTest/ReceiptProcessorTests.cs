...
namespace fiskaltrust.Middleware.Localization.QueueIT.UnitTest
{
    public class ReceiptProcessorTests
    {
        ...
        public async Task ReceiptProcessor_ThrowException_ReturnErrorResponse()
        {
            ...
            var sut = new ReceiptProcessor(LoggerFactory.Create(x => { }).CreateLogger<ReceiptProcessor>(), Mock.Of<IConfigurationRepository>(MockBehavior.Strict), Mock.Of<ILifecyclCommandProcessor>(MockBehavior.Strict), Mock.Of<IReceiptCommandProcessor>(MockBehavior.Strict), Mock.Of<IDailyOperationsCommandProcessor>(MockBehavior.Strict), Mock.Of<IInvoiceCommandProcessor>(MockBehavior.Strict), Mock.Of<IProtocolCommandProcessor>(MockBehavior.Strict), "cashBoxIdentification");
