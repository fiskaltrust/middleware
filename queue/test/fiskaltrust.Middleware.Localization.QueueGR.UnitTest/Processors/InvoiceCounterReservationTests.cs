...
public class InvoiceCounterReservationTests
{
    ...
    [Fact]
    public async Task DuplicateShapedError_OnPaymentMethodFlow_DoesNotAdvanceTheCounter()
    {
        // Pay0x3005 goes to AADE's payment-methods endpoint, never SendInvoices: its
        // errors cannot describe the queue's invoice numbering, and its reservation is
        // never consumed on success either (no mark). Even a failure that carries a
        // 233-shaped payload must not advance — that would burn a number for good.
        // Better to skip a heal than to skip a number.
        var queue = TestHelpers.CreateQueue();
        var queueGR = new ftQueueGR
        {
            ftQueueGRId = queue.ftQueueId,
        ...
        var storageProviderMock = new Mock<IQueueStorageProvider>();
        storageProviderMock.Setup(x => x.GetReceiptReferencesIfNecessaryAsync(It.IsAny<ProcessCommandRequest>()))
        ...
        var request = BuildRequest(queue, ReceiptCase.Pay0x3005);
        request.ReceiptRequest.cbPreviousReceiptReference = "previous-invoice";
        request.ReceiptRequest.cbPayItems = [new PayItem { Amount = 100, ftPayItemCase = (PayItemCase) 0x0000_0001_0000_0000 }];

        var result = await processor.Pay0x3005Async(request);
    ...
    [Fact]
    public async Task DuplicateShapedError_OnDeliveryNoteCancellation_DoesNotAdvanceTheCounter()
    {
        // Voided delivery notes go to AADE's cancellation endpoint, never SendInvoices
        // — same reasoning as the payment-method flow: a 233-shaped failure there must
        // fail the receipt without touching the counter.
        var queue = TestHelpers.CreateQueue();
        var queueGR = new ftQueueGR
        {
            ftQueueGRId = queue.ftQueueId,
        ...
        var request = BuildRequest(queue, ReceiptCase.DeliveryNote0x0005);
        request.ReceiptRequest.ftReceiptCase = Models.Cases.ReceiptCaseFlagsExt.WithFlag(
            request.ReceiptRequest.ftReceiptCase.WithFlag(ReceiptCaseFlags.Void),
            Models.Cases.ReceiptCaseFlags.HasTransportInformation);

        var result = await processor.DeliveryNote0x0005Async(request);
