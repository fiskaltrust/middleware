...
namespace fiskaltrust.Middleware.Localization.QueueGR.UnitTest.Processors
{
    public static class TestHelpers
    {
        ...
        /// ftQueueGR with an initialized invoice counter for any queue id and accepts
        /// InsertOrUpdateQueueGRAsync writes. Use this when a processor takes the repo but
        /// the test doesn't assert on counter state.
        /// </summary>
        public static AsyncLazy<IConfigurationRepository> CreateConfigurationRepositoryStub(ftQueueGR? queueGR = null)
        {
            ...
            repo.Setup(x => x.GetQueueGRAsync(It.IsAny<Guid>()))
                .ReturnsAsync(queueGR ?? new ftQueueGR { CashBoxIdentification = "CB-A", InvoiceSeries = "CB-A" });
