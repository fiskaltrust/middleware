...
namespace fiskaltrust.Middleware.Localization.QueueBE.UnitTest
{
    public class ReceiptProcessorTests
    {
        ...
        public async Task ReceiptProcessor_ReturnNotSupported_ReturnErrorResponse()
        {
            ...
            var sut = new ReceiptProcessor(LoggerFactory.Create(x => { }).CreateLogger<ReceiptProcessor>(), Mock.Of<IMarketValidator>(), Mock.Of<ILifecycleCommandProcessor>(), Mock.Of<IReceiptCommandProcessor>(), Mock.Of<IDailyOperationsCommandProcessor>(), Mock.Of<IInvoiceCommandProcessor>(), Mock.Of<IProtocolCommandProcessor>());
            var result = await sut.ProcessAsync(receiptRequest, receiptResponse, new ftQueue { }, new ftQueueItem { });
