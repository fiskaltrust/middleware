...
namespace fiskaltrust.Middleware.Localization.QueueME.UnitTest.RequestCommandsTests
{
    public class YearlyClosingReceiptCommandTest
    {
        ...
        public async Task ExecuteAsync_MonthlyClosing_ValidResultAsync()
        {
            ...
            var cashDepositReceiptCommand = await TestHelper.InitializeRequestCommand<YearlyClosingReceiptCommand>(queue.ftQueueId, "TestTCRCodePos", actionJournalRep).ConfigureAwait(false);
            var requestResponse = await cashDepositReceiptCommand.ExecuteAsync(new InMemoryMESSCD("TestTCRCodePos"), queue, TestHelper.CreateReceiptRequest(0x44D5_0000_0000_0006), queueItem);
