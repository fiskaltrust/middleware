...
namespace fiskaltrust.Middleware.Localization.QueueME.UnitTest.RequestCommandsTests
{
    public class MonthlyClosingReceiptCommandTest 
    {
        ...
        public async Task ExecuteAsync_MonthlyClosing_ValidResultAsync()
        {
            ...
            var requestResponse = await cashDepositReceiptCommand.ExecuteAsync(new InMemoryMESSCD("TestTCRCodePos", "iic", "iicSignature"), queue, TestHelper.CreateReceiptRequest(0x44D5_0000_0000_0005), queueItem, queueME);
            ...
            requestResponse.ActionJournals.FirstOrDefault().Type.Should().Be(0x44D5_0000_0000_0005.ToString("x").ToUpper());
        }
    }
}
