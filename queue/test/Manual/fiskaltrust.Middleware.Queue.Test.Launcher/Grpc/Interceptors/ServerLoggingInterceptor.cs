...
namespace fiskaltrust.Middleware.Queue.Test.Launcher.Grpc.Interceptors
{
    public class ServerLoggingInterceptor : Interceptor
    {
        ...
        {
            var message = "grpc Request | Method type: {MethodType} | Method name: {MethodName} | Request type: {RequestType} | Response type: {ResponseType}";
            if (request != null)
            {
                message += " | Response:" + System.Environment.NewLine + "{ResponseContent}";
            }
        ...
        {
            var message = "grpc Response | Method type: {MethodType} | Method name: {MethodName} | Request type: {RequestType} | Response type: {ResponseType} | Status: {StatusCode} ({StatusDetail})";
            if (response != null)
            {
                message += " | Response:" + System.Environment.NewLine + "{ResponseContent}";
            }
