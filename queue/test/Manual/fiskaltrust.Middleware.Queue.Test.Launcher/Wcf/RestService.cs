using fiskaltrust.Middleware.Queue.Test.Launcher.Wcf.Formatting;
    ...
    public class RestService : IDisposable
    {
        ...
        public void ConfigureService(PackageConfiguration config, Type type, object instance, string url)
        {
            ...
            try
            {
                ...
                _host.Open();
                _logger.LogInformation("{Protocol}: {PackageName} ({PackageVersion}) - Endpoint: {EndpointUrl}, {EndpointUrlJson}, {EndpointUrlXml}", "REST Service", config.Package, config.Version, url, $"{url}/json", $"{url}/xml");
            }
            catch (Exception x)
            {
                _logger.LogError(x, "Failed to start {Protocol} {PackageName} ({PackageVersion}) - Endpoint: {EndpointUrl}", "REST Service", config.Package, config.Version, url);
            }
        }
