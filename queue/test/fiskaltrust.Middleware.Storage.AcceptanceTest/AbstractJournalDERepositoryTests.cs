...
namespace fiskaltrust.Middleware.Storage.AcceptanceTest
{
    public abstract class AbstractJournalDERepositoryTests : IDisposable
    {
        ...
        {
            var expectedEntries = StorageTestFixtureProvider.GetFixture().CreateMany<ftJournalDE>(10);
        ...
        public async Task GetAsync_WithId_ShouldReturn_TheElementWithTheGivenId_IfTheIdExists()
        {
            ...
            var actualEntry = await sut.GetAsync(expectedEntry.ftJournalDEId);
        ...
        {
            var expectedEntries = StorageTestFixtureProvider.GetFixture().CreateMany<ftJournalDE>(10).OrderByDescending(x => x.TimeStamp).ToList();
        ...
        {
            var expectedEntries = StorageTestFixtureProvider.GetFixture().CreateMany<ftJournalDE>(10).OrderBy(x => x.TimeStamp).ToList();
        ...
        public async Task GetByTimeStampAsync_ShouldReturnAllEntries_FromAGivenTimeStamp_WithTake_ShouldReturnOnlyTheSpecifiedAmountOfEntries()
        {
            ...
            var actualEntries = await ((IMiddlewareRepository<ftJournalDE>) sut).GetEntriesOnOrAfterTimeStampAsync(firstSearchedEntryTimeStamp, 2).ToListAsync();
