...
namespace fiskaltrust.Middleware.Storage.AcceptanceTest
{
    public abstract class AbstractJournalDERepositoryTests : IDisposable
    {
        ...
        {
            var expectedEntries = StorageTestFixtureProvider.GetFixture().CreateMany<ftJournalDE>(10).ToList();
            expectedEntries = await StorageTestFixtureProvider.SetDifferentTimestamps(expectedEntries);
