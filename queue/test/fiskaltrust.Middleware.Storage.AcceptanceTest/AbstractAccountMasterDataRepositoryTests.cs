...
{
    public abstract class AbstractAccountMasterDataRepositoryTests : IDisposable
    {
        public abstract Task<IMasterDataRepository<AccountMasterData>> CreateRepository(IEnumerable<AccountMasterData> entries);
        public abstract Task<IMasterDataRepository<AccountMasterData>> CreateReadOnlyRepository(IEnumerable<AccountMasterData> entries);
        ...
        {
            var expectedEntries = StorageTestFixtureProvider.GetFixture().CreateMany<AccountMasterData>(10);
        ...
        {
            var sut = await CreateReadOnlyRepository(new List<AccountMasterData>());
            var actualEntries = await sut.GetAsync();
        ...
        {
            var entries = StorageTestFixtureProvider.GetFixture().CreateMany<AccountMasterData>(10).ToList();
            var entryToInsert = StorageTestFixtureProvider.GetFixture().Create<AccountMasterData>();
        ...
        {
            var entries = StorageTestFixtureProvider.GetFixture().CreateMany<AccountMasterData>(1).ToList();
