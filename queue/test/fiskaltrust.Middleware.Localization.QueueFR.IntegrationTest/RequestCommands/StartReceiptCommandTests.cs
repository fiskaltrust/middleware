...
{
    public class StartReceiptCommandTests : IClassFixture<SignProcessorDependenciesFixture>
    {
        private readonly SignProcessorDependenciesFixture _fixture;
        public StartReceiptCommandTests(SignProcessorDependenciesFixture fixture) => _fixture = fixture;
        ...
        public async Task ExecuteAsync_ShouldCreateTrainingReceiptResponse_WhenRequestHasTrainingReceiptFlag()
        {
            ...
            var command = new StartReceiptCommand(_fixture.signatureFactoryFR);
        ...
        public async Task ExecuteAsync_ShouldCreateDefaultReceiptResponse_WhenRequestDoesNotHaveTrainingReceiptFlag()
        {
            ...
            actionJournals.Should().Contain(x => x.Message == $"QueueItem {queueItem.ftQueueItemId} start enable of Queue {_fixture.queueFR.ftQueueFRId}");
        ...
        public void Validate_ShouldReturnValidationError_WhenRequestDoesNotHaveValidPayItems()
        {
            ...
            result.ToList().Should().Contain(x => x.Message == "The Start receipt must not have charge items.");
            result.ToList().Should().Contain(x => x.Message == "The Start receipt must not have pay items.");
        }
    }
}
