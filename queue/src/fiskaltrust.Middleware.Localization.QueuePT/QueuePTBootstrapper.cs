...
public class QueuePTBootstrapper : IV2QueueBootstrapper
{
    ...
    public QueuePTBootstrapper(Guid id, ILoggerFactory loggerFactory, Dictionary<string, object> configuration)
    {
        ...
        var signProcessorPT = new ReceiptProcessor(loggerFactory.CreateLogger<ReceiptProcessor>(), new LifecycleCommandProcessorPT(storageProvider.ConfigurationRepository), new ReceiptCommandProcessorPT(ptSSCD, queuePT, storageProvider.MiddlewareQueueItemRepository), new DailyOperationsCommandProcessorPT(), new InvoiceCommandProcessorPT(ptSSCD, queuePT, signaturCreationUnitPT, storageProvider.MiddlewareQueueItemRepository), new ProtocolCommandProcessorPT(ptSSCD, queuePT, signaturCreationUnitPT, storageProvider.MiddlewareQueueItemRepository));
