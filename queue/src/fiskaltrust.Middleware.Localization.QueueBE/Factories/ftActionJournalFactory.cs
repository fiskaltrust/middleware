...
public static class ftActionJournalFactory
{
    ...
    {
        var notification = new ActivateQueueBE
        {
        ...
        return CreateActionJournal(receiptResponse.ftQueueID, $"{request.ftReceiptCase:X}-{nameof(ActivateQueueBE)}", receiptResponse.ftQueueItemID, $"Initial-Operation receipt. Queue-ID: {receiptResponse.ftQueueID}", JsonConvert.SerializeObject(notification));
    }
    ...
    {
        var notification = new DeactivateQueueBE
        {
            CashBoxId = request.ftCashBoxID!.Value,
            QueueId = receiptResponse.ftQueueID,
            Moment = DateTime.UtcNow,
        ...
        return CreateActionJournal(receiptResponse.ftQueueID, $"{request.ftReceiptCase:X}-{nameof(DeactivateQueueBE)}", receiptResponse.ftQueueItemID, $"Out-of-Operation receipt. Queue-ID: {receiptResponse.ftQueueID}", JsonConvert.SerializeObject(notification));
    }
