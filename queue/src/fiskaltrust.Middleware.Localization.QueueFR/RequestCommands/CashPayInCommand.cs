...
{
    public class CashPayInCommand : RequestCommand
    {
        public CashPayInCommand(ISignatureFactoryFR signatureFactoryFR) : base(signatureFactoryFR) { }
        ...
        public override IEnumerable<ValidationError> Validate(ftQueue queue, ftQueueFR queueFR, ReceiptRequest request, ftQueueItem queueItem)
        {
            ...
            {
                yield return new ValidationError { Message = $"The Cash Deposit receipt must not have charge items." };
            }
        }
    }
}
