...
{
    public class ProtocolAccountingAuditCommand : RequestCommand
    {
        public ProtocolAccountingAuditCommand(ISignatureFactoryFR signatureFactoryFR) : base(signatureFactoryFR) { }
        ...
        public override IEnumerable<ValidationError> Validate(ftQueue queue, ftQueueFR queueFR, ReceiptRequest request, ftQueueItem queueItem)
        {
            ...
            {
                yield return new ValidationError {Message = $"The Audit receipt must not have charge items." };
            }
            if (request.cbPayItems != null && request.cbPayItems.Length > 0)
            {
                yield return new ValidationError { Message = $"The Audit receipt must not have pay items." };
            }
        }
    }
}
