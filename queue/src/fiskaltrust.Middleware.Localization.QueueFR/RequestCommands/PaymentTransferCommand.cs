...
{
    public class PaymentTransferCommand : RequestCommand
    {
        public PaymentTransferCommand(ISignatureFactoryFR signatureFactoryFR) : base(signatureFactoryFR) { }
        ...
        public override IEnumerable<ValidationError> Validate(ftQueue queue, ftQueueFR queueFR, ReceiptRequest request, ftQueueItem queueItem)
        {
            ...
            {
                yield return new ValidationError { Message = $"The Payment Transfer receipt must not have charge items." };
            }
        }
    }
}
