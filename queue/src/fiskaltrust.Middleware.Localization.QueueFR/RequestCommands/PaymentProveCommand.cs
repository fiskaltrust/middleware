...
{
    public class PaymentProveCommand : RequestCommand
    {
        public PaymentProveCommand(ISignatureFactoryFR signatureFactoryFR) : base(signatureFactoryFR) { }
        ...
        public override Task<(ReceiptResponse receiptResponse, ftJournalFR journalFR, List<ftActionJournal> actionJournals)> ExecuteAsync(ftQueue queue, ftQueueFR queueFR, ftSignaturCreationUnitFR signaturCreationUnitFR, ReceiptRequest request, ftQueueItem queueItem)
        {
            ...
            else
            {
                ...
                {
                    signatureItem,
                    new SignaturItem() { Caption = "justificatif non valable pour encaissement", Data = queue.ftQueueId.ToString(), ftSignatureFormat = (long)SignaturItem.Formats.Text, ftSignatureType = 0x4652000000000000 }
                };
        ...
        public override IEnumerable<ValidationError> Validate(ftQueue queue, ftQueueFR queueFR, ReceiptRequest request, ftQueueItem queueItem)
        {
            ...
            {
                yield return new ValidationError { Message = $"The Payment Prove receipt must not have charge items." };
            }
        }
    }
}
