...
namespace fiskaltrust.Middleware.Localization.QueueAT.RequestCommands
{
    public class PosReceiptCommand : RequestCommand
    {
        ...
        {
            var (receiptIdentification, ftStateData, isBackupScuUsed, signatureItems, journalAT, _) = await SignReceiptAsync(queueAT, request, response.ftReceiptIdentification, response.ftReceiptMoment, queueItem.ftQueueItemId);
