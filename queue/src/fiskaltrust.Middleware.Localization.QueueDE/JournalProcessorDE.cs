...
namespace fiskaltrust.Middleware.Localization.QueueDE
{
    public class JournalProcessorDE : IJournalProcessor
    {
        ...
        public async IAsyncEnumerable<JournalResponse> ProcessAsync(JournalRequest request)
        {
            ...
            if (request.ftJournalType == (long) JournalTypes.TarFileExport)
            {
                await foreach (var value in ProcessTarFileExportAsync(request).ConfigureAwait(false))
                {
                    yield return value;
                }
            }
            ...
            else if (request.ftJournalType == (long) JournalTypes.TarFileRangeExport)
            {
                await foreach (var value in ProcessTarFileRangeExportAsync(request).ConfigureAwait(false))
                {
                    yield return value;
                }
            }
        ...
        private async IAsyncEnumerable<JournalResponse> ProcessTarFileRangeExportAsync(JournalRequest request)
        {
        ...
        private async IAsyncEnumerable<JournalResponse> ProcessTarFileExportAsync(JournalRequest request)
        {
            var exportFilename = "";
            try
            {
