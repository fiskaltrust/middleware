...
using fiskaltrust.Middleware.Localization.QueueEU.Models;
...
public static class ftActionJournalFactory
{
    ...
    {
        var notification = new ActivateQueueEU
        {
        ...
        return CreateActionJournal(receiptResponse.ftQueueID, $"{request.ftReceiptCase:X}-{nameof(ActivateQueueEU)}", receiptResponse.ftQueueItemID, $"Initial-Operation receipt. Queue-ID: {receiptResponse.ftQueueID}", JsonConvert.SerializeObject(notification));
    }
    ...
    {
        var notification = new DeactivateQueueEU
        {
            CashBoxId = request.ftCashBoxID!.Value,
            QueueId = receiptResponse.ftQueueID,
            Moment = DateTime.UtcNow,
        ...
        return CreateActionJournal(receiptResponse.ftQueueID, $"{request.ftReceiptCase:X}-{nameof(DeactivateQueueEU)}", receiptResponse.ftQueueItemID, $"Out-of-Operation receipt. Queue-ID: {receiptResponse.ftQueueID}", JsonConvert.SerializeObject(notification));
    }
