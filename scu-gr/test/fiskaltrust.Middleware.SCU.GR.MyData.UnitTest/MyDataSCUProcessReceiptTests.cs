...
{
    [Fact]
    public async Task ProcessReceiptAsync_OnSuccess_LeavesReceiptIdentificationUntouched()
    {
        // The queue is the single writer of the "{series}-{aa}" segment in
        // ftReceiptIdentification: the SCU must never append or rewrite it. This pins
        // the invariant the queue's counter commit and the queue-start migration seed
        // rely on — a reintroduced append (as this code had for years) would produce
        // "ft1#CB-A-42CB-A-42" and corrupt the next migration seed.
