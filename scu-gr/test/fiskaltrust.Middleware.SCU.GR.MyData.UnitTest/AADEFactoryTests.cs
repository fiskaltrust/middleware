...
public class AADEFactoryTests
{
    ...
    [Fact]
    public void MapToInvoicesDoc_WithTipAsChargeItem_MultiplePaymentMethods_ShouldSubtractTipFromLargestPayment()
    {
        // Arrange -
        // Restaurant scenario: €300 bill + €30 tip, paid with €200 card + €130 cash
        // Tip should be subtracted from the largest payment (card) to avoid working with decimal values
        var receiptMoment = DateTime.Parse("2025-01-24T10:00:00.000Z").ToUniversalTime();
        var cbReceiptReference = Guid.NewGuid().ToString();
        ...
        cardPayment!.amount.Should().Be(170, "card payment (largest) has tip subtracted: 200 - 30 = 170");

        var cashPayment = invoice.paymentMethods.FirstOrDefault(p => p.paymentMethodInfo == "Cash Payment");
        cashPayment.Should().NotBeNull();
        cashPayment!.amount.Should().Be(130, "cash payment stays as-is");

        var totalReportedPayment = invoice.paymentMethods.Sum(p => p.amount);
        totalReportedPayment.Should().Be(300, "total payments should match invoice gross value (tip excluded)");
