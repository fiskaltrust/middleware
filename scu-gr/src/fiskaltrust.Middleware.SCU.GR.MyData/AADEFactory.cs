...
public class AADEFactory
{
    ...
    private static void ApplyMyDataOverride(AadeBookInvoiceType invoice, MyDataOverride overrideData)
    {
        ...
        // === COUNTERPART OVERRIDES (unmapped fields only) ===
        ...
        // === ISSUER OVERRIDES (unmapped fields only) ===
    ...
    private static void ApplyInvoiceHeaderOverride(AadeBookInvoiceType invoice, InvoiceHeaderOverride headerOverride)
    {
        ...
        // Apply reverse delivery note purpose
    ...
    public static void ApplyInvoiceDetailOverride(InvoiceRowType row, InvoiceDetailOverride detailOverride)
    {
        ...
        // Classification overrides (multi-value) replace the automatically determined values
        if (detailOverride.IncomeClassification != null)
        {
            row.incomeClassification = detailOverride.IncomeClassification
                .Select(MapIncomeClassificationOverride).ToArray();
        }
        if (detailOverride.ExpensesClassification != null)
        {
            row.expensesClassification = detailOverride.ExpensesClassification
                .Select(MapExpensesClassificationOverride).ToArray();
        }
    }

    private static IncomeClassificationType MapIncomeClassificationOverride(IncomeClassificationOverride ic)
    {
    ...
    private static ExpensesClassificationType MapExpensesClassificationOverride(ExpensesClassificationOverride ec)
    {
    ...
    private static List<InvoiceRowType> GetInvoiceDetails(ReceiptRequest receiptRequest)
    {
        ...
        return chargeItems.Select(grouped =>
        {
            ...
            if (x.ftChargeItemCaseData != null)
            {
                ...
                {
                    // ftChargeItemCaseData may contain data for other purposes (not mydataoverride), ignore errors
                }
            }
