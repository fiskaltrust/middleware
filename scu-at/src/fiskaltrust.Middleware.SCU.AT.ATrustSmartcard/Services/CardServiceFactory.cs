...
namespace fiskaltrust.Middleware.SCU.AT.ATrustSmartcard.Services
{
    public class CardServiceFactory
    {
        ...
        public ICardService CreateCardService(SCardContext cardContext, string readerName)
        {
            var cardState = cardContext.GetReaderStatus(readerName);
            if (cardState.Atr == null)
            {
                throw new Exception($"The card in the specified reader with the name '{readerName}' was not found.");
            }
